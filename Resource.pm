...
sub new {
	...
	if ($opts{id}) {
		...
		my $searched_name = lc($opts{name});
		my $rv = $dbh->selectall_hashref("SELECT * FROM resources WHERE resource_name LIKE \'%$searched_name%\'", 'resource_id');
...
sub reference_linking {
	...
	if (! $reference_linking && $reference_linking ne '1' && $reference_linking ne '0') {
...
sub delete {
	...
	if ($self->{resource_id}) {
		...
		use MyLibrary::Rank;
		my $rnk_cleanup_rtn = MyLibrary::Rank->cleanup(type => 'resource_delete', id => $self->{resource_id});
...
sub related_terms {
	...
	# make sure the resources has been saved to the database
	unless ($self->id() =~ /\d+/) { croak "No resource id found. Perhaps this resource object has not been saved to the database. Run commit() first." }
	...
	if (@new_related_terms) {
		TERMS:	foreach my $new_related_term (@new_related_terms) {
			...
			if ($strict_relations eq 'on') {
				my %term_list = ();
				map { $term_list{$_} = 1 } @{ $dbh->selectcol_arrayref('SELECT term_id FROM terms') };
				my $found_term;
				if ($term_list{$new_related_term}) { $found_term = 1; }
				unless ($found_term) { next TERMS }
			}
			my $found;
			my @found_term = $dbh->selectrow_array("SELECT term_id from terms_resources WHERE resource_id = ? AND term_id = ?", undef, $new_related_term, $self->id());
			if ($found_term[0]) { $found = 1 }
			if ($found) {
				next TERMS;
			} else {
				my $return = $dbh->do('INSERT INTO terms_resources (term_id, resource_id) VALUES (?,?)', undef, $new_related_term, $self->id());
				if ($return > 1 || ! $return) { croak "Unable to update term=>resource relational integrity. $return rows were inserted." }
			}
		}
	}
	
	# delete old term relations 
	if (@del_related_terms) {
		foreach my $del_related_term (@del_related_terms) {
			my $return = $dbh->do('DELETE FROM terms_resources WHERE resource_id = ? AND term_id = ?', undef, $self->id(), $del_related_term);
			if ($return > 1 || ! $return) { croak "Unable to delete term=>resource association. $return rows were deleted." }
			# remove rank associations
			use MyLibrary::Rank;
			my $rnk_cleanup_rtn = MyLibrary::Rank->cleanup(type => 'resource_term_change', id => $self->id(), term_id => $del_related_term);
			unless ($rnk_cleanup_rtn == 1) { croak "Unable to clean up data integrity for ranking tables in related_terms()." }
		}
	}
	
	my $return_term_ids;
	$return_term_ids = $dbh->selectcol_arrayref("SELECT term_id FROM terms_resources WHERE resource_id = ?", undef, $self->id());
