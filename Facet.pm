...
sub delete {
	...
	if ($self->{facet_id}) {
		...
		use MyLibrary::Rank;
		my $rnk_cleanup_rtn = MyLibrary::Rank->cleanup(type => 'facet_delete', id => $self->{facet_id});
...
sub related_terms {
	...
	if ($sort && $sort eq 'name') {
		$related_terms = $dbh->selectcol_arrayref("SELECT term_id FROM terms WHERE facet_id = ? ORDER BY term_name",undef,$self->facet_id());
		@related_terms = ();
		foreach my $related_term (@$related_terms) {
			push (@related_terms, $related_term);
		}
	} else {
		$related_terms = $dbh->selectcol_arrayref("SELECT term_id FROM terms WHERE facet_id = ?",undef,$self->facet_id());
		@related_terms = ();
		foreach my $related_term (@$related_terms) {
			push (@related_terms, $related_term);
		}
