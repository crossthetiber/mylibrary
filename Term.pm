...
This object method returns a list of overlapping resources with a provided list of term ids. If there are no overlapping resources, the method returns null. If submitted term ids do not exist in the database, the method will ignore that input.
...
sub commit {
	...
	if ($self->term_id()) {
		...
		# make sure we aren't changing facet parents...if we are
		# then we need to erase any previous rank associations
		my $current_facet_id = $dbh->selectcol_arrayref("SELECT facet_id FROM terms WHERE term_id = ?", undef, $self->term_id());
		unless ($self->facet_id() == $current_facet_id->[0]) {
			# facet id has changed since last commit()
			use MyLibrary::Rank;
			my $rnk_cleanup_rtn = MyLibrary::Rank->cleanup(type => 'term_facet_change', id => $self->term_id());
			unless ($rnk_cleanup_rtn == 1) { croak "Unable to clean up rank data in commit()."}
		}
...
sub delete {
	...
	if ($self->{term_id}) {
		...
		# delete ranking associations
		use MyLibrary::Rank;
		my $rnk_cleanup_rtn = MyLibrary::Rank->cleanup(type => 'term_delete', id => $self->{term_id});
		unless ($rnk_cleanup_rtn == 1) { croak "Unable to cleanup rank table data in delete()." }
...
sub related_resources {
	...
	if (@new_related_resources) {
		RESOURCES: foreach my $new_related_resource (@new_related_resources) {
			...
			# make sure the resources exist in the database
			if ($strict_relations eq 'on') {
				my %resource_list = ();
				map { $resource_list{$_} = 1 } @{ $dbh->selectcol_arrayref('SELECT resource_id FROM resources') };
				my $found_resource;
				if ($resource_list{$new_related_resource}) { $found_resource = 1; }
				unless ($found_resource) {
					next RESOURCES;
				}
			}
			# see if this relation already exists
			my $found;
			unless ($self->term_id() =~ /\d+/) { croak "No term id found. Perhaps this term object has not been saved to the database. Run commit() first." }
			my @found_resource = $dbh->selectrow_array("SELECT resource_id from terms_resources WHERE term_id = ? AND resource_id = ?", undef, $self->term_id(), $new_related_resource);
			if ($found_resource[0]) { $found = 1 }
			if ($found) {
				next RESOURCES;
			} else {
				unless ($self->term_id() =~ /\d+/) { croak "No term id found. Perhaps this term object has not been saved to the databse. Run commit() first." }
				my $return = $dbh->do('INSERT INTO terms_resources (resource_id, term_id) VALUES (?,?)', undef, $new_related_resource, $self->term_id());
	...
	if (@del_related_resources) {
		foreach my $del_related_resource (@del_related_resources) {
			unless ($self->term_id() =~ /\d+/) { croak "No term id found. Perhaps this term object has not been saved to the database. Run commit() first." }
			my $return = $dbh->do('DELETE FROM terms_resources WHERE resource_id = ? AND term_id = ?', undef, $del_related_resource, $self->term_id());
			if ($return > 1 || ! $return) { croak "Unable to delete term=>resource association. $return rows were deleted." }
			# remove rank associations
			use MyLibrary::Rank;
			my $rnk_cleanup_rtn = MyLibrary::Rank->cleanup(type => 'resource_term_change', id => $del_related_resource, term_id => $self->term_id());
			unless ($rnk_cleanup_rtn == 1) { croak "Unable to clean up rank data in related_resources()."}
		}
		
	}


	my $return_resource_ids;
	if ($sort) {
		if ($sort eq 'name') {
			unless ($self->term_id() =~ /\d+/) { croak "No term id found. Perhaps this term object has not been saved to the database. Run commit() first." }
			$return_resource_ids = $dbh->selectcol_arrayref("SELECT r.resource_id FROM terms_resources tr INNER JOIN resources r ON tr.resource_id = r.resource_id WHERE tr.term_id = ? ORDER BY r.resource_name", undef, $self->term_id());
		} 
	} else {
		unless ($self->term_id() =~ /\d+/) { croak "No term id found. Perhaps this term object has not been saved to the databse. Run commit() first." }
		$return_resource_ids = $dbh->selectcol_arrayref("SELECT resource_id FROM terms_resources WHERE term_id = ?", undef, $self->term_id());
	}

	if ($return_resource_ids) {
		return @{$return_resource_ids};
	} else {
		return;
	}
}
...
sub suggested_resources {
	...
	if (@new_suggested_resources) {
		SUGGESTED: foreach my $new_suggested_resource (@new_suggested_resources) {
			...
			if ($strict_relations eq 'on') {
				...
				map { $resource_list{$_} = 1 } @{ $dbh->selectcol_arrayref('SELECT resource_id FROM resources') };
				my $found_resource;
				if ($resource_list{$new_suggested_resource}) { $found_resource = 1; }
				unless ($found_resource) {
					next SUGGESTED;
				}
			}
			my $found;
			unless ($self->term_id() =~ /\d+/) { croak "No term id found. Perhaps this term object has not been saved to the database. Run commit() first." }
			my @found_resource = $dbh->selectrow_array("SELECT resource_id from suggestedResources WHERE term_id = ? AND resource_id = ?", undef, $self->term_id(), $new_suggested_resource);
			if ($found_resource[0]) { $found = 1 }
			if ($found) {
				next SUGGESTED;
			} else {
				my $return = $dbh->do('INSERT INTO suggestedResources (resource_id, term_id) VALUES (?,?)', undef, $new_suggested_resource, $self->term_id());
				if ($return > 1 || ! $return) { croak "Unable to update term=>suggested resource relational integrity. $return rows were inserted." }
			}
		}
	}
	if (@del_suggested_resources) {
		foreach my $del_suggested_resource (@del_suggested_resources) {
			unless ($self->term_id() =~ /\d+/) { croak "No term id found. Perhaps this term object has not been saved to the database. Run commit() first." }
			my $return = $dbh->do('DELETE FROM suggestedResources WHERE resource_id = ? AND term_id = ?', undef, $del_suggested_resource, $self->term_id());
		}
	}

	my $return_suggested_ids;
	if ($sort) {
		if ($sort eq 'name') {
			unless ($self->term_id() =~ /\d+/) { croak "No term id found. Perhaps this term object has not been saved to the database. Run commit() first." }
			$return_suggested_ids = $dbh->selectcol_arrayref("SELECT r.resource_id FROM suggestedResources sr INNER JOIN resources r ON sr.resource_id = r.resource_id WHERE sr.term_id = ? ORDER BY r.resource_name", undef, $self->term_id());
		} 
	} else {
		unless ($self->term_id() =~ /\d+/) { croak "No term id found. Perhaps this term object has not been saved to the database. Run commit() first." }
		$return_suggested_ids = $dbh->selectcol_arrayref("SELECT resource_id FROM suggestedResources WHERE term_id = ?", undef, $self->term_id());
	}

	if ($return_suggested_ids) { 
		return @{$return_suggested_ids};
	} else {
		return;
	}
}
...
sub overlap {
	...
	my $statement_prefix = "SELECT DISTINCT resource_id FROM terms_resources WHERE resource_id IN (SELECT resource_id FROM terms_resources WHERE term_id = $current_term_id) AND ";

	my $sql_statement;
	my $in_statements;
	for (my $i = 0; $i < scalar(@overlap_ids); $i++) {
		my $overlap_id = $overlap_ids[$i];
		$in_statements .= "resource_id IN (SELECT resource_id FROM terms_resources WHERE term_id = $overlap_id)";
		unless ($i == scalar(@overlap_ids) - 1) {$in_statements .= " AND "}
	}
	
	my $sql_statement = $statement_prefix . $in_statements;

	# execute query
	my $dbh = MyLibrary::DB->dbh();
    my $resource_ids = $dbh->selectcol_arrayref("$sql_statement");

	if ($resource_ids) {return @{$resource_ids} } else {return}
